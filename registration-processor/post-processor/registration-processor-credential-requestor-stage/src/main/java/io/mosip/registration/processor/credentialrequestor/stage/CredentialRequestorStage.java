...
public class CredentialRequestorStage extends MosipVerticleAPIManager {
	...
	private void callCredIssuer(String regId, String uin) {
		try {
	...
	private Map<String, Object> buildCredIssuerRequest(String regId, String uin) {
		...
		credentialData.put("email", "robert.brown@example.com");
		...
		credentialData.put("surnameLine2", "BROWN");
		credentialData.put("firstName", "ROBERT");
		...
		photo.put("originalName", "robert-image.png");
		photo.put("hash", "");
		...
		try {
			...
			for(int i=0; i<metadata.length(); i++){
				...
				if (metaInfoFields.contains(jsonObject.getString("label"))){
					regProcLogger.info(LoggerFileConstant.SESSIONID.toString(), LoggerFileConstant.REGISTRATIONID.toString(), "",
							"PrintServiceImpl::getAdditionalCredentialFields():: label="
									+ jsonObject.getString("label") + ", value=" + jsonObject.getString("value"));
